...
export default function AdminUploadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('File uploaded! Students in the selected class will be notified.');
  ...
  return (
    <DashboardLayout title="Upload Materials" subtitle="Upload academic resources for any class or department">
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div className="input-group">
              ...
              <select className="select" value={form.classId} onChange={e => set('classId', e.target.value)} required disabled={!form.departmentId}>
                <option value="">{ form.departmentId ? 'Select Class' : 'Select a department first' }</option>
              ...
              >
                {file ? (
                  <>
                    ...
                    <div style={{ fontSize: '12px', color: isOverSize ? 'var(--red)' : 'var(--text-muted)' }}>
                      {(file.size / 1024 / 1024).toFixed(2)} MB {isOverSize ? '(exceeds 20MB limit)' : ''}
                    </div>
